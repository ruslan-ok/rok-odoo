...
from odoo.addons.documents.models.documents_document import (
    Document as Document_for_patching,
)
from odoo.addons.documents_spreadsheet.models.documents_document import (
    SUPPORTED_PATHS,
    XLSX_MIME_TYPES,
)
...
class Document(models.Model):
    ...
    located_on_the_server = fields.Boolean(
        "This object (folder or file) is located on the Server",
        default=False,
    )
    ...
    def _register_hook(self):
        ...
        Document_for_patching.search_panel_select_range = (
            search_panel_select_range_patch
        )
        ...
        if field_name == "folder_id":
            ...
            if unique_folder_id := self.env.context.get("documents_unique_folder_id"):
                values = self.env["documents.document"].search_read(
                    expression.AND(
                        [domain, [("folder_id", "child_of", unique_folder_id)]],
                    ),
                    search_panel_fields,
                    order="name asc",
                )
                ...
                for record in values:
                    if folder_id := record["folder_id"]:
                        record["folder_id"] = (
                            folder_id[0]
                            if folder_id[0] in accessible_folder_ids
                            else False
                        )
                return {
            ...
            for record in records:
                ...
                if folder_id:
                    ...
                    record["owner_id"][0] != self.env.ref("base.user_root").id
    ...
    def web_search_read(
        ...
        if (
            ...
            and domain[0] == "&"
            and len(domain[1]) == 3
            and domain[1][0] == "folder_id"
            and domain[1][1] == "="
            and len(domain[2]) == 3
            and domain[2][0] == "owner_id"
            and domain[2][1] == "="
        ):
    ...
    def create(self, vals_list):
        ...
        if isinstance(folder_id, int):
            ...
            if folder.located_on_the_server:
                recordset = recordset.with_context(default_located_on_the_server=True)
        documents = super(Document, recordset).create(vals_list)
    ...
    def refresh_server_folder(self):
        ...
        for file in child_files:
            if (
                file.type == "binary"
                and not file._exist_on_the_server()
                and not file.spreadsheet_data
            ):
    ...
    def _unzip_xlsx(self):
        ...
        with zipfile.ZipFile(file) as input_zip:
            ...
            for info in input_zip.infolist():
                if not (
                    info.filename.endswith((".xml", ".xml.rels"))
                    or "media/image" in info.filename
