...
class XadesSigner:
    ...
    def __init__(self, cert_content, certificate_password):
        if not cert_content:
        ...
        try:
            ...
            raise UserError(
                _("The provided file is not a valid certificate/private key."),
            )
    ...
    def _get_xml_to_sign(self, challenge_code, nip):
        ...
        return etree.fromstring(xml_to_sign_str)
    ...
    def _build_qualifying_properties(self, signature_node, sig_id, props_id):
        ...
        etree.SubElement(
            cert_digest_node,
            etree.QName(NS_DS, "DigestValue"),
        ).text = base64.b64encode(cert_digest.finalize()).decode("utf-8")

        issuer_serial_node = etree.SubElement(
            cert_node,
            etree.QName(NS_XADES, "IssuerSerial"),
        )
        etree.SubElement(
            issuer_serial_node,
            etree.QName(NS_DS, "X509IssuerName"),
        ).text = self.cert.issuer.rfc4514_string()
        etree.SubElement(
            issuer_serial_node,
            etree.QName(NS_DS, "X509SerialNumber"),
    ...
    def sign_authentication_challenge(self, challenge_code, nip):
        ...
        digest2_node.text = self._build_qualifying_properties(
            signature_node,
            sig_id,
            props_id,
        )
        ...
        etree.SubElement(
            signature_node,
            etree.QName(NS_DS, "SignatureValue"),
        ...
        etree.SubElement(
            x509_data,
            etree.QName(NS_DS, "X509Certificate"),
