...
from odoo.tools import file_open, float_is_zero, float_repr, xml_utils
...
class AccountMove(models.Model):
    ...
    l10n_pl_move_reference_number = fields.Char(
        string="KSeF Invoice Reference Number",
        readonly=True,
    )
    ...
    def _check_mandatory_fields(self):
        ...
        if not company.vat:
            errors.append(
                _("You must have a VAT number to be able to issue e-invoices\n"),
            )
        if not company.country_id or (
            not company.street and not company.city and not company.zip
        ):
            errors.append(_("Please complete the address of your company\n"))
        return errors

    def _get_ksef_invoice_type(self):
        """
        Determines the specific TRodzajFaktury for KSeF.
        """
        self.ensure_one()

        def is_downpayment_line(line):
            if getattr(line, "is_downpayment", False):
                return True
            if hasattr(line, "sale_line_ids"):
                return any(
                    getattr(sl, "is_downpayment", False) for sl in line.sale_line_ids
                )
            return False
        ...
        if self.move_type == "out_refund":
            ...
            origin_type = origin_inv._get_ksef_invoice_type()
            if origin_type == "ZAL":
                return "KOR_ZAL"
            if origin_type == "ROZ":
                return "KOR_ROZ"
        ...
        if self.move_type == "out_invoice":
            has_dp_lines = any(
                is_downpayment_line(line) for line in self.invoice_line_ids
            )

            if has_dp_lines:
                # Check for Settlement (ROZ): Contains Regular lines AND Negative Downpayment lines
                has_regular_lines = any(
                    not is_downpayment_line(line) for line in self.invoice_line_ids
                )
                has_deducted_dp = any(
                    is_downpayment_line(line) and line.price_subtotal < 0
    ...
    def _get_ksef_related_invoices(self):
        ...
        """
        self.ensure_one()
        ksef_type = self._get_ksef_invoice_type()
        related_numbers = []
            ...
            if hasattr(self.line_ids, "sale_line_ids"):
                ...
                for so in sale_orders:
                    ...
                    for prev in previous_invoices:
                        ...
                        if prev._get_ksef_invoice_type() == "ZAL":
                            related_numbers.append(prev.name)

        return list(set(related_numbers))
    ...
    def _l10n_pl_ksef_get_xml_values(self):
        ...
        def get_amounts_from_tag(tax_tag_string):
            if "OSS" in tax_tag_string:
                oss_tag = self.env["account.account.tag"].search(
                    [("name", "=", "OSS"), ("applicability", "=", "taxes")],
                )
                return -sum(
                    self.line_ids.filtered(
                        lambda line: any(tag in oss_tag for tag in line.tax_tag_ids)
                        and (
                            line.tax_ids
                            if "Base in tax_tag_string"
                            else not line.tax_ids
                        ),
                    ).mapped("amount_currency"),
                )
            tax_tags = self.env["account.account.tag"]._get_tax_tags(
                tax_tag_string,
                self.env.ref("base.pl").id,
            )
            return -sum(
                self.line_ids.filtered(
                    lambda line: any(tag in tax_tags for tag in line.tax_tag_ids),
                ).mapped("amount_currency"),
            )
        ...
        def compute_p_12(move_line):
            ...
            if "eksport" in tags_str or "export" in tags_str:
                return "0 EX"  # Export of goods

            if "odwrotne" in tags_str or "oo" in name_str:
                return "oo"  # Reverse Charge

            if "niepodlegaj" in tags_str or "np" in name_str:
                return "np I"  # Not subject to tax (assuming Type I as default)

            if "zw" in name_str or "zwolnion" in tags_str:
                return "zw"  # Exempt
            ...
            invoice_lines_vals.append(
                {
                    ...
                    "P_11": format_monetary(line.price_subtotal * sign),
                    "P_12": p_12_value,
                },
            )
    ...
    def _l10n_pl_ksef_render_xml(self):
        ...
        try:
            with file_open("addons/l10n_pl_edi/data/fa3_schema.xsd", "rb") as xsd:
                xml_utils._check_with_xsd(xml_content, xsd)
        except Exception as e:
            _logger.error(
                "KSeF XML schema validation failed for invoice %s: %s",
                self.name,
                str(e),
            )
            raise
    ...
    def action_download_l10n_pl_ksef_xml(self):
        ...
        if not xml_content_bytes:
            raise UserError(_("Generated XML content is empty."))
            ...
            attachment = self.env["ir.attachment"].create(
                {
                    ...
                    "res_id": self.id,
                    "datas": xml_datas,
                    "description": _("KSeF FA(3) Invoice XML"),
                },
            )
    ...
    def _get_ksef_status_mapping(self):
        ...
        """
        return {
            200: ("accepted", _("KSeF Status: Success (Code: 200). Invoice accepted.")),
            150: ("sent", _("KSeF Status: Processing in progress (Code: 150).")),
            100: (
                "sent",
                _("KSeF Status: Invoice accepted for further processing (Code: 100)."),
            ),
            405: (
                "rejected",
                _("KSeF Status: Rejected (Code: 405). Processing canceled."),
            ),
            410: (
                "rejected",
                _("KSeF Status: Rejected (Code: 410). Incorrect scope of permissions."),
            ),
            415: (
                "rejected",
                _(
            ...
            435: (
                "rejected",
                _("KSeF Status: Rejected (Code: 435). File decryption error."),
            ),
            440: (
                "rejected",
                _("KSeF Status: Rejected (Code: 440). Duplicate invoice."),
            ),
            450: (
                "rejected",
                _(
            ...
            500: ("rejected", _("KSeF Status: Error (Code: 500). Unknown error.")),
        }
    ...
    def action_update_invoice_status(self):
        ...
        if not self.l10n_pl_move_reference_number:
            raise UserError(
                _(
                    "This invoice does not have a KSeF Invoice Reference Number or Session ID. It may not have been sent yet.",
                ),
            )
        ...
        try:
            response = service.get_invoice_status(
                self.l10n_pl_move_reference_number,
                company.l10n_pl_ksef_session_id,
            )
        except UserError as e:
            self.message_post(body=_("Failed to check KSeF status: %s", str(e)))
            return

        _logger.info("KSeF status check response for move %s: %s", self.name, response)

        l10n_pl_ksef_number = response.get("ksefNumber", {})
        ...
        if status_code in mapping:
            ...
            new_status = self.ksef_status
            message = _(
                "Received an unknown or unexpected status from KSeF (Code: %(code)s): %(description)s",
                code=status_code,
                description=status_description,
            )
            _logger.warning(
                "Unknown KSeF status code '%s' for move %s",
                status_code,
                self.name,
            )
    ...
    def action_get_invoice_UPO(self):
        ...
        service = KsefApiService(
            self.company_id,
            mode,
            self.company_id.l10n_pl_ksef_session_id,
            self.company_id.l10n_pl_ksef_session_key,
            self.company_id.l10n_pl_ksef_session_iv,
        )
        ...
        try:
            ...
            self.message_post(body=_("Failed to download UPO: %s", str(e)))
            return None
        if isinstance(upo_xml_content, str):
            upo_xml_content_bytes = upo_xml_content.encode("utf-8")
        else:
            upo_xml_content_bytes = upo_xml_content

        if not upo_xml_content_bytes:
            raise UserError(_("The KSeF service returned empty UPO content."))

        dom = parseString(upo_xml_content_bytes)
        ...
        if attachment:
            attachment.write({"datas": upo_datas})
            attachment_id = attachment.id
        else:
            attachment = self.env["ir.attachment"].create(
                {
                    ...
                    "res_id": self.id,
                    "datas": upo_datas,
                    "description": _("KSeF Official Confirmation of Receipt (UPO)"),
                },
            )
            attachment_id = attachment.id
