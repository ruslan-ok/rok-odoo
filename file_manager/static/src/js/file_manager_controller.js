...
import { FileManagerSidebar } from '@file_manager/components/sidebar/sidebar';
...
export class FileManagerFormController extends FormController {
    static template = "file_manager.FileManagerFormView";
    static components = {
    ...
    setup() {
        ...
        */
        ...
        useChildSubEnv({
            ...
            ensureFolderName: this.ensureFolderName.bind(this),
        ...
        useBus(this.env.bus, 'KNOWLEDGE:OPEN_ARTICLE', (event) => {
            this.openFolder(event.detail.id);
        });
        ...
        onWillStart(() => {
            if (
                ...
                // this.fileManagerCommandsService.unregisterCommandsRecordInfo(this.env.config.breadcrumbs);
            }
        });
        onMounted(() => {
     ...
     * the folder from inside an folder (i.e. embedded views/links) very
     ...
     * save the whole folder, if the current folder exists (it does
     * not exist if there are no folders to show, in which case the no
    ...
    /**
     * If the folder has no name set, tries to rename it.
     */
    ensureFolderName() {
     ...
     * When an folder has no name set, use the title (first h1 in the
     * body) to try to save the folder with a name.
     * @overwrite
     */
    ...
    /**
     * @param {integer} - resId: id of the folder to open
     */
    async openFolder(resId) {
        ...
        try {
            ...
            this.dialogService.add(AlertDialog, {
                ...
                body: _t(
                    "The folder you are trying to open has either been removed or is inaccessible.",
                ),
    ...
    /*
     * Rename the folder using the given name, or using the folder title if
     * no name is given (first h1 in the body). If no title is found, the
     * folder is kept untitled.
    ...
    toggleAsideMobile(force) {
        const container = this.root.el.querySelector('.o_file_manager_form_view');
