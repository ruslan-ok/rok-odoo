...
import {
    FileManagerSidebarFavoriteSection,
    FileManagerSidebarPrivateSection,
    FileManagerSidebarSharedSection,
    FileManagerSidebarWorkspaceSection
...
export class FileManagerSidebar extends Component {
    ...
    static template = "file_manager.Sidebar";
    static components = {
        FileManagerSidebarFavoriteSection,
        FileManagerSidebarPrivateSection,
        FileManagerSidebarSharedSection,
    ...
    setup() {
        ...
        this.storageKeys = {
            size: "file_manager.sidebarSize",
            unfoldedFolders: "file_manager.unfolded.ids",
            unfoldedFavorites: "file_manager.unfolded.favorite.ids",
        };
    ...
    async moveToTrash(folder) {
        ...
        if (this.isAncestor(folder.id)) {
            this.actionService.doAction(
                await this.orm.call('file.manager.folder', 'action_home_page', [false]),
                {stackPosition: 'replaceCurrentAction'}
            );
        } else {
            this.removeFolder(folder);
