...
class FileManagerFile(models.Model):
    ...
    def _compute_access_token(self):
        for file in self:
            file.access_token = f"{file.document_token}o{file.id or 0:x}"
    ...
    def get_content(self):
        ...
        file_path = os.path.join(root_path, self.folder_id.path, self.name)
    ...
    def _compute_thumbnail(self):
        for file in self:
            if file.mimetype and file.mimetype.startswith("application/pdf"):
                # Thumbnails of pdfs are generated by the client. To force the generation, we invalidate the thumbnail.
                file.thumbnail = False
                file.thumbnail_status = "client_generated"
            elif file.mimetype and file.mimetype.startswith("image/"):
                try:
                    content = file.get_content()
                    file.thumbnail = base64.b64encode(image_process(content, size=(200, 140), crop="center"))
                    file.thumbnail_status = "present"
                except (UserError, TypeError):
                    file.thumbnail = False
                    file.thumbnail_status = "error"
            else:
                file.thumbnail = False
                file.thumbnail_status = False
