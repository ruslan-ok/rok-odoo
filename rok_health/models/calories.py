...
class Calories(models.Model):
    ...
    info = fields.Html()
    product_or_activity = fields.Char(
        compute="_compute_product_or_activity",
        store=True,
    )
            ...
            match record.direction:
                ...
                case "consumed":
                    ...
                    if kcal_100g and consumed_g:
                        ...
                        if pack_pcs:
                            record.consumed_kcal = (
                                pack_g / 100 * kcal_100g / pack_pcs * consumed_pcs
                            )
                        else:
                            record.consumed_kcal = (
                                pack_g / 100 * kcal_100g * consumed_pcs
                            )
                    else:
    ...
    def _compute_product_or_activity(self):
        for record in self:
            match record.direction:
                case "burned":
                    record.product_or_activity = dict(
                        record._fields["activity"].selection,
                    ).get(record.activity, record.activity)
                case "consumed":
