import { registry } from "@web/core/registry";
import { _t } from "@web/core/l10n/translation";
import { useInputField } from "@web/views/fields/input_field_hook";
import { CharField } from "@web/views/fields/char/char_field";
import { standardFieldProps } from "@web/views/fields/standard_field_props";
import { CopyButton } from "@web/core/copy_button/copy_button";
import { ShowHideButton } from "../../buttons/show_hide_button/show_hide_button";
import { GeneratePasswordButton } from "../../buttons/generate_password_button/generate_password_button";

import { useState, useRef, Component } from "@odoo/owl";
import { PasswordGenerator } from "../../dialogs/password_generator_dialog";

export class PasswordField extends Component {
    static template = "password_manager.ListPasswordField";
    static components = { Field: CharField, CopyButton };
    static props = {
        ...standardFieldProps,
        placeholder: { type: String, optional: true },
    };

    setup() {
        useInputField({ getValue: () => this.props.record.data[this.props.name] || "" });
        this.successText = _t("Copied");
        this.sensitiveHiddenValue = "**********";
        this.sensitiveValue = this.props.name === "value";
        this.state = useState({ hideSensitive: true });
        this.input = useRef("input");
    }

    get inputType() {
        if (this.sensitiveValue && this.state.hideSensitive)
            return "password";
        return "text";
    }

    get listFieldValue() {
        if (this.sensitiveValue && this.state.hideSensitive)
            return this.sensitiveHiddenValue;
        return this.props.record.data[this.props.name];
    }

    get inputType() {
        if (this.sensitiveValue && this.state.hideSensitive)
            return "password";
        return "text";
    }

    get copyButtonIcon() {
        return "fa-clone";
    }

    get copyButtonClassName() {
        return `o_btn_copy btn-sm`;
    }
}

export const passwordField = {
    component: PasswordField,
    displayName: _t("Password"),
};

registry.category("fields").add("list_password", passwordField);

class FormPasswordField extends PasswordField {
    static components = { CopyButton, ShowHideButton, GeneratePasswordButton };
    static template = "password_manager.FormPasswordField";

    setup() {
        super.setup();
        // Auto-generate initial value for new records (only once)
        if (
            this.props.name === "value" &&
            this.props.record.isNew &&
            !this.props.record.data[this.props.name] &&
            !this._autoGenerated
        ) {
            this._autoGenerated = true;
            // Read preferences from localStorage (fallbacks align with dialog defaults)
            const get = (k) => localStorage.getItem(k);
            const isPassphrase = get("is_passphrase") === "1";
            const passwordLength = +(get("password_length") || 15);
            const numberOfWords = +(get("number_of_words") || 6);
            const state = {
                value: "",
                isPassphrase,
                passwordLength: isPassphrase ? numberOfWords : passwordLength,
                minimumNumbers: +(get("minimum_numbers") || 1),
                minimumSpecials: +(get("minimum_specials") || 1),
                wordSeparator: get("word_separator") || "-",
                capitalize: get("capitalize") === "1",
                includeNumber: get("include_number") === "1",
                avoid: get("avoid") === "1",
            };
            // Build use map like in dialog (upper always true)
            const use = isPassphrase
                ? { upper: true, lower: true, numbers: true, special: true, avoid: true }
                : {
                    upper: true,
                    lower: get("include_lower") === "1",
                    numbers: get("include_numbers") === "1",
                    special: get("include_special") === "1",
                    avoid: state.avoid,
                };
            const generator = new PasswordGenerator(state, use);
            const generated = generator.generate();
            // Update record asynchronously; ignore autosave to avoid premature save
            Promise.resolve().then(() => this.useGeneratedValue(generated));
        }
    }

    toggleSensitive() {
        this.state.hideSensitive = !this.state.hideSensitive;
        this.input.type = "text";
    }

    get showHideButtonIcon() {
        if (this.state.hideSensitive)
            return "fa-eye-slash";
        return "fa-eye";
    }

    get showHideButtonClassName() {
        return `o_btn_show_hide btn-sm`;
    }

    get generatePasswordButtonIcon() {
        return "fa-rotate-right";
    }

    async useGeneratedValue(value) {
        await this.props.record.update(
            { [this.props.name]: value },
            { save: this.props.autosave }
        );
    }
}

export const formPasswordField = {
    ...passwordField,
    component: FormPasswordField,
};

registry.category("fields").add("form_password", formPasswordField);
